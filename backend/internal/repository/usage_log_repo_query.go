...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		refundedCredits           sql.NullFloat64
		createdAt                 time.Time
	)
	...
	if refundedAt.Valid {
		...
		log.RefundedAt = &value
	}
	if refundedCredits.Valid {
		log.RefundedCredits = &refundedCredits.Float64
	}
