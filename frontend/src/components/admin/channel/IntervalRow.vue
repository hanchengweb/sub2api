<template>
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <!-- Token mode: context range + prices（单位是积分/百万 token，不是美元）-->
    <template v-if="mode === 'token'">
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ t('common.creditUnit') }}/M</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ t('common.creditUnit') }}/M</span></label>
        <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWritePriceShort') }} <span class="text-gray-300">{{ t('common.creditUnit') }}/M</span></label>
        <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPriceShort') }} <span class="text-gray-300">{{ t('common.creditUnit') }}/M</span></label>
    ...
    <template v-else>
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.perRequestPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ t('common.creditUnit') }}</span>
          <span v-if="yuanHint(interval.per_request_price)" class="ml-1 text-gray-400">{{ yuanHint(interval.per_request_price) }}</span></label>
