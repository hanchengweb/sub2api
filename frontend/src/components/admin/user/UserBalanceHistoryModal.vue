      ...
      <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-shrink-0 text-right">
            ...
            <p class="text-xl font-bold text-gray-900 dark:text-white">
              {{ accountCurrencySymbol }}{{ user.balance?.toFixed(2) || '0.00' }}
            </p>
          </div>
        </div>
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-gray-200/60 pt-2.5 dark:border-dark-600/60">
          ...
          <p class="ml-4 flex-shrink-0 text-xs text-gray-500 dark:text-dark-400">
            {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-emerald-600 dark:text-emerald-400">{{ accountCurrencySymbol }}{{ totalRecharged.toFixed(2) }}</span>
          </p>
        </div>
      </div>
...
const formatValue = (item: BalanceHistoryItem) => {
  if (isBalanceType(item.type)) {
    ...
    return `${sign}${accountCurrencySymbol}${item.value.toFixed(2)}`
  }
