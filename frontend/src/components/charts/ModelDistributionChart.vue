            ...
            <template v-for="model in displayModelStats" :key="model.model">
              ...
              >
                ...
                <td class="py-1.5 text-right text-green-600 dark:text-green-400">
                  {{ formatCost(model.actual_cost) }} {{ t('common.creditUnit') }}
                </td>
                <td v-if="showAccountCost" class="py-1.5 text-right text-orange-500 dark:text-orange-400">
                  {{ formatCreditsAsMoney(model.account_cost, creditsPerCurrencyUnit) }}
                </td>
                <td class="py-1.5 text-right text-gray-400 dark:text-gray-500">
                  {{ formatCost(model.cost) }} {{ t('common.creditUnit') }}
                </td>
              </tr>
        ...
        <table class="w-full text-xs">
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-1.5 text-right text-green-600 dark:text-green-400">
                {{ formatCost(item.actual_cost) }} {{ t('common.creditUnit') }}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      callbacks: {
        label: (context: any) => {
          ...
          const formattedValue = props.metric === 'actual_cost'
            ? `${formatCost(value)} ${t('common.creditUnit')}`
            : formatTokens(value)
    ...
    tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          const total = context.dataset.data.reduce((a: number, b: number) => a + b, 0)
          const percentage = total > 0 ? ((value / total) * 100).toFixed(1) : '0.0'
          return `${context.label}: ${formatCost(value)} ${t('common.creditUnit')} (${percentage}%)`
        }
      }
    }
  }
