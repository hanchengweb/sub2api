    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400">{{ CURRENCY_SYMBOL }}{{ formatBalance(balance) }}</p>
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-gray-900 dark:text-white">
            <span class="text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">{{ CURRENCY_SYMBOL }}{{ formatCost(stats?.today_actual_cost || 0) }}</span>
            <span class="text-sm font-normal text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / {{ CURRENCY_SYMBOL }}{{ formatCost(stats?.today_cost || 0) }}</span>
          </p>
          <p class="text-xs">
            <span class="text-gray-500 dark:text-gray-400">{{ t('common.total') }}: </span>
            <span class="text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">{{ CURRENCY_SYMBOL }}{{ formatCost(stats?.total_actual_cost || 0) }}</span>
            <span class="text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / {{ CURRENCY_SYMBOL }}{{ formatCost(stats?.total_cost || 0) }}</span>
          </p>
        </div>
      </div>
    </div>
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <span class="font-mono text-sm text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">
            {{ CURRENCY_SYMBOL }}{{ formatCost(item.total_actual_cost) }}
          </span>
        </div>
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</span>
            <span class="font-mono text-gray-900 dark:text-white">{{ CURRENCY_SYMBOL }}{{ formatCost(item.today_actual_cost) }}</span>
          </div>
          <div class="flex items-center justify-between">
        ...
        <!-- Quota 区：仅当 quota 配置存在且至少有一个窗口配了 limit 时显示 -->
        <div v-if="hasAnyLimit(item.quota)" class="mt-3 space-y-1.5 border-t border-gray-200 pt-2 dark:border-dark-700">
              ...
              <template v-else>
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span class="font-mono text-gray-700 dark:text-gray-200">
                    {{ CURRENCY_SYMBOL }}{{ formatUsd((quotaVal(item.quota, `${w}_usage_usd`) as number) ?? 0) }} / {{ CURRENCY_SYMBOL }}{{ formatUsd(quotaVal(item.quota, `${w}_limit_usd`) as number) }}
                  </span>
                </div>
                <div class="h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700">
                  <div
...
import { getDomesticModelProvider, type DomesticModelProvider } from '@/utils/domesticModels'

interface FusedPlatformCard {
  platform: DomesticModelProvider
...
const props = defineProps<{
  ...
  todayModels?: ModelStat[] | null
}>()
const { t } = useI18n()
const CURRENCY_SYMBOL = currencySymbol()
const DOMESTIC_PLATFORM_ORDER: DomesticModelProvider[] = [
  'deepseek', 'qwen', 'zhipu', 'kimi', 'doubao', 'minimax', 'wenxin', 'spark',
  'hunyuan', 'baichuan', 'internlm', 'yi', 'stepfun'
]

const platformLabel = (platform: DomesticModelProvider) => t(`dashboard.domesticPlatforms.${platform}`)

const platformCards = computed<FusedPlatformCard[]>(() => {
  const cards = new Map<DomesticModelProvider, FusedPlatformCard>()
  let hasDomesticModelStats = false
  let hasDomesticTodayModelStats = false
  const ensureCard = (platform: DomesticModelProvider) => {
    const existing = cards.get(platform)
    if (existing) return existing
    const card: FusedPlatformCard = {
      platform,
      total_actual_cost: 0,
      today_actual_cost: 0,
      total_requests: 0,
  ...
  for (const model of props.models ?? []) {
    const platform = getDomesticModelProvider(model.model)
    if (!platform) continue
    hasDomesticModelStats = true
    const card = ensureCard(platform)
    card.total_actual_cost += model.actual_cost
    card.total_requests += model.requests
    card.total_tokens += model.total_tokens
  }

  for (const model of props.todayModels ?? []) {
    const platform = getDomesticModelProvider(model.model)
    if (!platform) continue
    hasDomesticTodayModelStats = true
    ensureCard(platform).today_actual_cost += model.actual_cost
  }

  // Older deployments may return domestic platform IDs directly. Fill only the
  // dimensions that are missing from model statistics to avoid double-counting.
  if (!hasDomesticModelStats || !hasDomesticTodayModelStats) {
    for (const item of props.stats?.by_platform ?? []) {
      const platform = getDomesticModelProvider(item.platform)
      if (!platform) continue
      const card = ensureCard(platform)
      if (!hasDomesticModelStats) {
        card.total_actual_cost += item.total_actual_cost
        card.total_requests += item.total_requests
        card.total_tokens += item.total_tokens
      }
      if (!hasDomesticTodayModelStats) card.today_actual_cost += item.today_actual_cost
    }
  }

  for (const quota of props.platformQuotas ?? []) {
    const platform = getDomesticModelProvider(quota.platform)
    if (!platform) continue
    ensureCard(platform).quota = quota
  }

  return [...cards.values()].sort((a, b) => DOMESTIC_PLATFORM_ORDER.indexOf(a.platform) - DOMESTIC_PLATFORM_ORDER.indexOf(b.platform))
})
