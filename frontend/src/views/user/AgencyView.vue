  ...
  <AppLayout>
    <div class="agency-page">
      <section class="agency-hero" aria-labelledby="agency-headline">
        ...
        <img src="/agency/partnership-bridge.webp" alt="" width="677" height="260" class="agency-bridge" />
      </section>
      ...
      <div class="agency-workspace">
        ...
        <form ref="formElement" class="agency-form" @submit.prevent="submitApplication" @input="clearValidity">
 ...
 * 真正把申请提交到平台。
 *
 * 成功后立刻刷新「我的申请」：没有回执的话，用户不知道提交成没成，
 * 只会反复再点一次——服务端那道待处理上限挡的就是这个。
 */
async function submitApplication() {
  ...
  try {
    ...
    submitError.value = code === 'TOO_MANY_PENDING'
      ? t('agency.tooManyPending')
      : t('agency.submitFailed')
  } finally {
...
async function loadMyApplications() {
  try {
    myApplications.value = await listMyAgencyApplications()
  } catch {
    // 拉不到历史不影响提交，静默即可——这不是主路径
  }
}
