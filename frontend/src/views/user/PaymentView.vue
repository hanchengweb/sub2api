            ...
            <div v-if="validAmount > 0" class="card p-6">
              <div class="space-y-2 text-sm">
                ...
                <div v-if="balanceRechargeMultiplier !== 1" class="flex justify-between" :class="{ 'border-t border-gray-200 pt-2 dark:border-dark-600': feeRate <= 0 }">
                  ...
                  <span class="text-gray-900 dark:text-white">{{ creditedAmount.toFixed(2) }} {{ t('common.creditUnit') }}</span>
                </div>
                <p v-if="balanceRechargeMultiplier !== 1" class="border-t border-gray-200 pt-2 text-xs text-gray-500 dark:border-dark-600 dark:text-gray-400">
                  {{ t('payment.rechargeRatePreview', { credits: balanceRechargeMultiplier.toFixed(2) }) }}
                </p>
              </div>
            </div>
              ...
              <div class="card p-5">
                ...
                <div class="mt-3 grid grid-cols-2 gap-3">
                  ...
                  <div v-if="selectedPlan.daily_limit_usd != null">
                    ...
                    <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ selectedPlan.daily_limit_usd }} {{ t('common.creditUnit') }}</div>
                  </div>
                  <div v-if="selectedPlan.weekly_limit_usd != null">
                    <span class="text-xs text-gray-400 dark:text-gray-500">{{ t('payment.planCard.weeklyLimit') }}</span>
                    <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ selectedPlan.weekly_limit_usd }} {{ t('common.creditUnit') }}</div>
                  </div>
                  <div v-if="selectedPlan.monthly_limit_usd != null">
                    <span class="text-xs text-gray-400 dark:text-gray-500">{{ t('payment.planCard.monthlyLimit') }}</span>
                    <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ selectedPlan.monthly_limit_usd }} {{ t('common.creditUnit') }}</div>
                  </div>
...
async function createOrder(orderAmount: number, orderType: OrderType, planId?: number, options: CreateOrderOptions = {}) {
  ...
  try {
    ...
    if (decision.kind === 'redirect_waiting' && decision.paymentState.payUrl) {
      if (isMobileDevice() || decision.paymentState.paymentMode.trim().toLowerCase() === 'redirect') {
        window.location.href = decision.paymentState.payUrl
        return
      }
      openWindow(decision.paymentState.payUrl)
